...
export class OpenAI {
  ...
  resetConversation(conversationId: string) {
    if (conversationId) delete this.conversationHistoryObject[conversationId]
  }
  ...
  async generateCommand(response: Response): Promise<Response> {
    try {
      // 进行指令拆解
      const splitResult = await this.splitCommand(response)
      ...
      if (splitResult) {
        ...
        if (splitResult.contents && splitResult.contents.length > 0) {
          for (const commandPrompt of splitResult.contents) {
            ...
            if (commandPrompt.prompt) {
              const content = await this.convertCommand(
                response.conversationId,
                commandPrompt.prompt,
                commandPrompt.domain
              )
              if (content) {
                if (content.text) {
                  response.type = 'text'
  ...
  async splitCommand(response: Response): Promise<Response | undefined> {
    ...
    if (!response.conversationId) response.conversationId = result.data.id

    const retJson = this.extractJsonFromString(message.content)
  ...
  async convertCommand(
      ...
      extendData.forEach((item: any, index: number) => {
        ...
        if (item.score > 0.85 && !typeArray.includes(type)) {
          for (const key in text) {
            ...
            systemTextExtend +=
              indexText + '. ' + item.payload.title + '的' + key + '为' + text[key] + ', '
          }
          typeArray.push(type)
        }
      })
    ...
    const systemText = systemContexts[domain] || ''
  ...
  extractJsonFromString(text: string) {
    ...
    if (startIndex !== -1 && endIndex !== -1 && startIndex < endIndex) {
      ...
      try {
        ...
        console.error('JSON字符串内容:', jsonString)
